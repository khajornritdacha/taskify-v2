...
export default function App() {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    let tmp: string = '', currentTodos = [...todos], currentCompletedTodos = [...completedTodos];
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <main className="flex min-h-screen flex-col bg-bright-navy-blue font-neucha">
        ...
        <Task todos={todos} setTodos={setTodos} completedTodos={completedTodos} setCompletedTodos={setCompletedTodos}/> 
      </main>
    </DragDropContext>
  );
}
